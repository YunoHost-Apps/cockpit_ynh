...
myynh_fix_dynamicuser() {
	...
	add_user_unique() {
		local u="$1"
		[[ -z "$u" ]] && return
		if [[ -z "${created_users_set[$u]+_}" ]]; then
			created_users+=("$u")
	...
	if ! is_dynamicuser_supported; then
		for f in /usr/lib/systemd/system/cockpit*; do
			if grep -q '^DynamicUser=yes' "$f"; then
				...
				if [[ -n "$user" ]]; then
					if ! ynh_system_user_exists --username="$user"; then
				...
				if [[ -n "$group" ]]; then
					if ! ynh_system_group_exists --group="$group"; then
...
myynh_remove_non_dynamic_users() {
	if [[ "$(ynh_app_setting_get --key=is_dynamicuser)" == "0" ]]; then
...
is_dynamicuser_supported() {
	...
	if systemd-run --wait --quiet --unit "$unit_name" --property DynamicUser=yes /bin/true 2>/dev/null; then
